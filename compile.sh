...
python ${CLOSURE_DIR}/closure/bin/calcdeps.py -i app/requirements.js \
    ...
    -i ${APP_DIR}deps.js \
    -p ${CLOSURE_DIR}  -p ${CLOSURE_TEMPLATE_DIR} -p ${PLASTRONJS_DIR} -p ${APP_DIR} --output_file=dist/assets/js/hedgehog.app.min.js -c ./tools/closure-compiler/build/compiler.jar \
